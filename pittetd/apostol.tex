...
A_m - 2 & = & a^{2m}+1 - 2 \\
& = & a^{2m} - 1 \\
& = & (a^{2n} + 1)(a^{2m-2n} - a^{2m-2n-2n} + a^{2m - 2n - 2n - 2n} -  \ldots - 1)
\neea
%
so $(a^{2n} + 1)|(a^{2m} - 1)$ if $m = nk$ where $k$ is even.

If $A_n|(A_m - 2)$ and $d=\gcd(A_n,A_m) > 1$ then $d|(A_m-2)$ as well, this means that since $d|A_m$, $d|2$ so $d$ is either $2$ or $1$ depending on whether $a$ is even or odd, if $a$ is even $a^{2w} + 1$ is odd and so $d=1$ otherwise $d=2$.
...
\neea
%
so in this case $(a^{2n} + 1)|(a^{2m} + 1)$ if $m = nk$ where $k$ is {\it odd}.

%
\nbea
a^{2h}-1 & = & (a^{2j} + 1)(a^{2h-2j} - a^{2h-2j-2j} + \dots - 1)
\neea
%
so here $h = jw$ and $w$ must be even.
...
\neea
%
and the process continues with the same exact pattern as Euclid's algorithm to find gcd between two numbers and that's what we will find, the $\gcd(m,n) = g$, \ie we will eventually get $d|(a^{2g} \pm 1)$ we can of course continue one step further to get $d|(a^{2m}+1)\to d|2$ but this is not needed.

%
\nbea
d &|& ((a^{2(m-n)} - 1) + (a^{2n} + 1)) \\
d &|& a^{2(m-n)} + a^{2n} \\
d &|& a^{2n}(a^{2(m-n - n)} + 1) ~~~~~~~~~~~~{\rm if~} 2m-2n>2n \\
d &|& a^{2(m-n)}(a^{2(n-(m-n))} + 1) ~~~~~{\rm if~} 2m-2n<2n
\neea
%
...
we could've added $(a^{2m} + 1)$ to $(a^{2(m-n)} - 1)$ instead with similar final conclusion.

For either case if $d | a^{2n}$ or $d | a^{2(m-n)}$, just like above it'll mean that $d|1$ which is not interesting. The more interesting case is that we can repeat the process as many times as we like until we reach something like
%
\nbea
d &|& a^{2w}(a^{2(xm+yn)}+1)
\neea
%
where $xm+yn$ is such that $(a^{2(xm+yn)}+1)$ the smallest possible $a^{2u}+1$ that is divisible by $d$. Note that everytime we do the above procedure we will get $d|a^{2u}+1$ where $a^{2u} + 1$ is smaller than the one before.

Thanks to Bezout the smallest $xm+yn$ is of course $\gcd(m,n)$
