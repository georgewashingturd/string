...
Let's solve this the way a physicist might have done it. First, the whole situation is symmetrical under $a \leftrightarrow b$ and $h \leftrightarrow k$ and $ab \leftrightarrow hk$ therefore a first guess would be
%
\nbea
(ab,hk) & = & (a,h)(b,k)(a,k)(b,h)
\neea
%
but this immediately fails when $a=p,b=p,h=p,k=1$
%
\nbea
...
%
\nbea
(ab,hk) & = & (a,h)(b,k)\frac{(a,k)(b,h)}{(a,b)(h,k)}
\neea
%
we divide by both $(a,b)$ and $(h,k)$ because the $ab \leftrightarrow hk$ symmetry, but again this is overdoing it, for example for $a=pp,b=1,h=p,k=p$
%
\nbea
(pp\cdot1, p\cdot p) & = & (pp,p)(1,p)\frac{(pp,p)(1,p)}{(pp,1)(p,p)} \\
& = & (p)(1)\frac{(p)(1)}{(1)(p)} \\
& = & p
\neea
%
...
\neea
%
and that turns out to be correct :) There are two reasons I chose to do the above problem, one, it's fun to apply what I learned during my physics PhD on a math problem and two we will need this result to solve Problem 1.28.
...
{\bf Proposition 1.28.2}, $\gcd(x^n + x^{n-1} + \ldots + 1,x^m + x^{m-1} + \ldots + 1) = \sum_{i=0}^{\gcd(n+1,m+1)}x^{i}$


%
\nbea
(a^m-1,a^n-1) & = & ((a^d-1)f(m/d),(a^d-1)f(n/d)) \\
& = & (a^d-1,a^d-1)(f(m/d),f(n/d)) \left ( a^m-1,\frac{a^{n-1}-1}{(a-1,a^{n-1}-1)}\right ) \\
& = & (a-1) \left ( a^m-1,\frac{a^{n-1}-1}{a-1}\right ) \\
& = & (a-1) \left ( a^m-1,(a^{n-2} + a^{n-3} + \ldots + 1)\right ) \\
\neea
%
...
\nbea
(a^m-1,a^n-1) & = & ((a^d-1)f(m/d),(a^d-1)f(n/d)) \\
& = & (a^d-1,a^d-1)(f(m/d),f(n/d)) \times \\
&& \left ( \frac{a^d-1}{(a^d-1,f(m/d))}\frac{f(n/d)}{(a^d-1,f(n/d))}\right )\left ( \frac{f(m/d)}{(a^d-1,f(m/d))}\frac{a^d-1}{(a^d-1,f(n/d))}\right )
\neea
%

where $f(m/d) = \left ( (a^d)^{m/d-1} + (a^d)^{m/d-2} + \ldots + a^d + 1 \right )$, now let $g$ be a common divisor of $(a^d-1)$ and $f(m/d)$ this means that
%
\nbea
g &|& f(m/d) - (a^d - 1) \\
g &|& (a^d)^{m/d-1} + (a^d)^{m/d-2} + \ldots + 2 \\
\to g &|& a^d ((a^d)^{m/d-2} + (a^d)^{m/d-3} + \ldots + 2)
\neea
%
or more specifically this means $g|a^d$ but since $d|a^d-1$
%
\nbea
g &|& a^d - (a^d-1) \\
\to g &|& 1
\neea
%
therefore $(a^d-1,f(m/d)) = 1$, this is obviously true for $f(n/d)$ too, \ie $()$
