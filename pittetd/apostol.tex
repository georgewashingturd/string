...
\neea
%
for $a$ to have an integer solution $w^2+4n$ must be a perfect square and as described in Problem 3.20, $4n+j$ can be a perfect square if and only if $j=0,1$ so therefore as long as $w=0,1,2$ (which are the valid values for $w$) the above will have a solution but $w=0$ means that $4n$ is a perfect square which means that $n$ is a perfect square. Therefore as long as either $n$ is a perfect square or $4n+1$ or $4n+4$ is a perfect square, $n$ is divisible by $[\sqrt{n}]$.
...
8\cdot 17 + 13 & \equiv & -1 \pmod{25} \\
8\cdot 20 + 13 & \equiv & -2 \pmod{25} \\
8\cdot 23 + 13 & \equiv & -3 \pmod{25}
%\end{array}
\neea
%
First note $8\cdot 1 + 13 \equiv -4 \pmod{25}$ means that $8\cdot1+13$ is only 4 away (less than 8 away) from the next multiple of $25$, thus if we increase $n=1\to n=2$, the quotient increases by one, \ie $[(8\cdot1+13)/25]=0 \to [(8\cdot2+13)/25]=1$. This means that everytime $8n+13 \equiv -x\pmod{25}$ where $0 < x \le 8$, increasing $n$ by one increases the quotient by one as well.

Next thing to note is that the remainder $-x\pmod{25}$ will decrease to $-(x+1)\pmod{25}$ everytime $n\to n+3$ which makes sense since $8\cdot3 = 24$, adding one more 8 to 24 will cross 25 and will increase the quotient by 1 and therefore the quotient increases everytime $n\to n+3$ although a special care needs to be taken whenever $8n+13 \equiv 0\pmod{25}$, in this case increasing $n\to n+3$ does not increase the quotient.

As a concrete example, let's start with $n=2$, the quotient will only change when $n=2+3=5$, thus, $n=2,3,4$ all have the same quotient, \ie $[8\cdot2+13]=[8\cdot3+13]=[8\cdot4+13]=1$, the exception to this is at the beginning when $n=1$, the quotient changes when $n=1\to n=1+1$.

This whole pattern repeats every 25 counts, \ie $n \to n+25$, as
%
\nbea
8m + 13 & = & 25q + r \\
...
\neea
%
the quotient will of course increase but the remainder (which controls when the quotient changes) remains the same



means that if we add one more 8 we get $8\cdot2+13 \equiv (-4+8) \pmod{25} \equiv 4 \pmod{25}$ which means that the quotient increases
...
The first thing to note here is that starting from $n=2$ changing $n \to n+3$ we go from $-x \pmod{25} \to -(x+1) \pmod{25}$ this means that going from $n \to n+1\to n+2$ will not change $\left \lbrack \frac{8n+13}{25} \right \rbrack$ because
%
\nbea
