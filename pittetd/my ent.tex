...
\item First, we want to show that if $\mod{2^n}$ has the oscillating pattern as described above, $\mod{2^{n+1}}$ will also have the same oscillating pattern (with a lower frequency and an odd first quotient), this is to ensure the continual survival of our inductive step.
...
\neea
%
which is a contradiction unless $p=2$ (but we are talking about ). Therefore there are only 2 roots $\pm 1$. We also know that $x^{\varphi(p^n)}-1$ has exactly $\varphi(p^n)$ roots (from Euler's theorem), however
%
\nbea
...
The key now is to find a number inside $(\mathbb{Z}/p^n\mathbb{Z})^*$ that has an order $(p-1)$. This is {\it very} tricky. The way I proceeded was suppose we have a number $x$ that has an order $a \mod m$ and another order $b \mod n$, what can we say about the order of $x \mod (mn)$? Well
%
\nbea
x^y & \equiv & 1 \pmod{mn} \\
& = & 1 + mn\cdot k \\
...
But now we need to take care of the cases for $x^{p^m h}$ where $h | (p-1)$, $0 < m < n-1$. First
%
\nbea
...
With this result it is true that $x^{p^m h} \not\equiv 1 \pmod{p^{n+1}}$, $0 < m < n-1, ~h|(p-1)$ because otherwise it would violate  Proposition 2.2.7.(b).1. We have therefore shown that the order of $x$ is $pq_n = p^n \mod p^{n+1}$.
...
We can still show that 3 is a generator but we need to tweak the proof a bit. Hence we start with $5 = 1 + 2^2$ and $\mod 2^2$ instead. However, by the above argument $1 - 2^2$ also works but this is just $-3$ and since the other generator is $-1$, $3$ is also a generator.
...
\neea
%
which is a contradiction and thus every time we increase $q$ the remainder changes, except for $n = 2$ where $5^q \mod 2^2$ is always 1.
