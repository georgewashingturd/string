...
(b) Prove that $a_n$ is divisible by 11 if and only if $n \equiv 1~ ({\rm mod~} 6)$.
...
& = & a - b + c - d + \dots ~ ({\rm mod~} 11) \\
0 & = & a - b + c - d + \dots ~ ({\rm mod~} 11)
\neea
%
the last line is because $11|n \to n = 0~({\rm mod~}11)$, and so the alternate sum of the digits of the number must be zero for the number to be a multiple of 11.
...
I tried to show that the fact that $p$ and $p^2 + 2$ are prime implies $p=3$ and it didn't work out too well. The best approach is to flip the statement, \ie if $p \neq 3$ then $p$ is not prime OR $p^2 + 2$ is not prime (we can't have both to be prime) which is the solution given in the book.

As a side note, you might be tempted to generalize this into, $p$ and $p^2 + (p-1)$, \eg $p=5$ with $p^2 + (5-1) = 29$ where both are primes. However, for $p=7$, $p^2 + 6 = 55$ is not prime. The problem is that $5^2 + 6 = 31, ~19^2 + 6 = 367$ are also prime.
