...
{\bf Page 114, Problem 3}. We need to verify $\eta^p \equiv \eta \pmod{p}$ in the case of $\lambda = 7$, $p = 2$ and then for $\lambda = 13$ and $p = 13$. First, $\lambda = 7$ here $\lambda - 1 = 6 = 2\cdot 3$. So we can either have $e = 2, f = 3$ or $e = 3, f = 2$. But $\eta^p \equiv \eta$ only works if $\eta$ is a period of length $f$ where $f$ is the exponent (or I rpefer to call it order) of $p \pmod{\lambda}$.
...
This means that $k^\lambda \equiv 1 \pmod{p}$, therefore it's either $k = 1$ (the only element with order one is $k = 1$) or the order of $k$ mod $p$ is $\lambda$ (because $\lambda$ is prime otherwise the order can be any factor of $\lambda$). But $k = 1$ is already covered above and we have explictly assumed that $k\neq1$ in this case so that we were able to use the geometric series formula so the order of $k$ is $\lambda$. Then Fermat's Little Theorem tells us that $\lambda|p - 1$ and we immediately have $p \equiv 1 \pmod{\lambda}$.
...
In this case $Nh(\alpha) = 19$ so it can't be $0 \pmod{\lambda}$. Thereofore it must be $1 \pmod{\lambda = 5}$ but 19 is not $1 \pmod{19}$ thus contrary to our asuumption, $\eta_0 - 4$ is irreducible. The same arguments goes for $\eta_1 - 4$. There are also other factorizations like $(\eta_0 + 5)(\eta_1 + 5) = 19$ and by the same argument above each factor is also irreducible.
