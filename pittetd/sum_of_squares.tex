...
So if we use the same inductive reasoning as above, what we want to show is that for a prime $p$ where $-n$ is a quadratic residue we have
%
\nbea
r^2 + n & = & pw
\neea
...
%
\nbea
p^2 - r^2 & \ge & p^2 - p^2 + 2p - 1 \\
\to p^2 - r^2 & \ge & 2p - 1
\neea
% 
so if $n < 2p - 1$ then $w < p$. For $1 \le n \le 5$, it is $ < 2p-1$ for any prime $p \ge 5$, thus the induction proof should still work.

For other $n$ it still works it's just that the inductive basis requires more work. The difference here is that $w$ contains primes where $-n$ is a quadratic residue although just because $-n$ is a quadratic residue doesn't mean that it is expressible as $x^2 + ny^2$.

So in our inductive assumption we need to prove that this is so.
