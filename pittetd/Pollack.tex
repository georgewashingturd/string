...
\neea
%
but somehow $\vartheta(x) \sim x$, and all this is due to the error term when $x$ is not prime, if we are more careful
%
\nbea
...
and since $\pi(x) \sim x/\log x$, $\vartheta(x) \sim x$ of course here we are fortunate enough that the error is not too big or too small.

More interesting is $\psi(x)$, it is very similar to $\vartheta(x)$ except that it also includes all powers of primes so the naive estimate is $\psi(x) \sim \log x$ and this time it is close to $\log x$ because we include all powers of $p \le x$ but in actuality
%
\nbea
...
The expansion for $\pi/4$ I believe is from the expansion of atan
