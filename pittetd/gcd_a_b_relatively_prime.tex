...
One of my strategies is to start with $4x + a$ now if $a$ is zero we can always move it to $a = 3$ (zero is the worst because then $x$ can be anything mod 4).

$a = 3$ is the best because the factors of $4x + 3$ can only be $-1, 1 \mod 4$, for $4y + b$ we can always choose $b = 2$ so that the factors can only be $-1,-2 \mod 4$ thus the only common factor is $-1 \mod 4$ and therefore the gcd is 1.

The trick now is to make sure that the factor must be $-2 \mod 4$ rather than $2 \mod 4$ although they are the same
