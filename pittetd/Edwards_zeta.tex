...
The branch point here is $z=1$ because here the function stops being multi-valued. We then express all other points with respect to $z = 1$. Note that nnumbers in the complex plane are like vectors, so if we choose $z=1$ as a reference, all other points can be expressed as $z = 1 + r e^{i\alpha}$ where $r = |z - 1|$ and $\alpha$ is the angle with respect to the branch cut.
