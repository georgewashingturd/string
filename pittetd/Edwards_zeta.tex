...
\neea
%
because we estimate the denominator using $|s|$, we have to do the $\frac{d}{ds}$ derivative here, otherwise we incur a minus sign and need to do the complicated $\frac{d|s|}{ds}$ later. So
%
\nbea
\frac{d}{ds} R_{2\nu-2} & \le & -\int_0^\infty \frac{[B_{2\nu} - B_{2\nu}(x)]~ dx}{(s+x)^{2\nu+1}}
\neea
...
\neea
%
combining the two we get something like the middle of Page 112 (and also forgetting about the second integral)
%
\nbea
...
\neea
%
Note that the minus sign in the second line is crucial as it negates the minus sign from computing hte lower limit of the integral :) if we set $\nu=1$ we get
%
\nbea
...
\neea
%
for sufficiently large $s$ and this is the key, sufficiently large
%
\nbea
...
\left | \re\frac{\Pi'(s)}{\Pi(s)} - \re\log s \right | & \le & \frac{1}{|s|}
\neea
%
...
\neea
%
since whatever is in the bracket is less than 1 and so we get the final result :)
