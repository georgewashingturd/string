...
{\bf Page 113}. ``the desired result $A = \frac{1}{2}\log 2\pi$ follows by taking the limit as $t\to\infty$''. The constant $A$ has nothing to do with $t$ per se, here we're taking the limit $t\to\infty$ as a trick to get rid of the unwanted terms, that's all, it's just a trick.
...
{\bf Page 115}, top. Error estimate. From line 1 to 2 we do IBP with $\int \frac{1}{2\nu}\overline{B}_{2\nu - 1} = B_{2\nu} - \overline{B}_{2\nu}(x)$ as on Page 111 bot. The boundary terms of the IBP, $\left.[B_{2\nu} - \overline{B}_{2\nu}(x)] x^{-s-2\nu+1}\right|_N^\infty$, are zero because
%
\begin{enumerate}
\item $B_{2\nu} = \overline{B}_{2\nu}(x)$ for $x$ integer because $\overline{B}_{2\nu}(x)$ is the periodic version and $B_{2\nu} = \overline{B}_{2\nu}(0)$, this takes care of the lower limit.
%
\item For the upper limit we need $\re~s + 2\nu - 1 > 0$ such that $\frac{1}{x^{s + 2\nu - 1}} = 0$ as $x\to\infty$, but this can always be achieved since $\nu$ is up to us, it is the number of terms we want to include in the Euler-Maclaurin expansion, so this is always satisfied.
\end{enumerate}
%
