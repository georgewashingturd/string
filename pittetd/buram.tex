...
Some important observation, if some $n + dx_i$ is divisible by some $p_{j_u} \in \mathbb{N}_x$ then any $n + dx_i + dp_{j_u}y$ is also divisible by $p_{j_u}$.
...
In this way the bits containing $p_{j_u}$ is $(u + sp_{j_u})$, \ie bit position $u \pmod{p_{j_u}}$ will all contain $p_{j_u}$. Once we assign each bit a prime from $\mathbb{N}_x$ we have the following assignments
%
\nbea
\begin{array}{r c c c c c}
{\rm ~bit:~} & 0 & + & s_0 p_{j_0} & \to & p_{j_0} \\
{\rm ~bit:~} & 1 & + & s_1 p_{j_1} & \to & p_{j_1} \\
& & \vdots & & \\
{\rm ~bit:~} & t-1 & + & s_{t-1} p_{j_{t-1}} & \to & p_{j_{t-1}}
\end{array}
\neea
%
note that bit 0 is just $n$ and since $(n,k) > 1$, $n$ must contain one of the primes. The remaining question is whether there is a bit that wasn't represented by the above assignment and there is, thanks to Chinese Remainder Theorem


Total number of bit is $\ge p_{j_i} p_{j_2} \dots p_{j_t}$
